...
for i in range(0, 4):
	...
	for j in range(3, -1, -1):
		tmp.append(data[i + (4*j)])
	rooms.append(tuple(tmp))
rooms = tuple(rooms)
room_size = 4
	...
	def calculate(hallway: Tuple[str, None], rooms: Tuple[Tuple[str]]):
		...
		for idx_pod, current_pod in enumerate(hallway): # move hallway pod to room
			...
			if can_enter_room(rooms, hallway, idx_pod):
				...
				distance += (room_size + 1) - len(new_rooms[target_room_idx]) # required step to get into room
		...
		# if all pod in room in the right room: continue
		# else pop the top pod
		for idx_room, pods_in_room in enumerate(rooms):
			if len(pods_in_room) == 0 \
				or all(pod == final_state[idx_room][0] for pod in pods_in_room): continue
			else:
				...
				for hall_position in avaliable_hall_position:
					if all(pod_in_hallway == None
						...
						distance += (room_size) - len(new_rooms[idx_room]) # required step to get out the room
